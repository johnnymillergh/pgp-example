...
import com.jmsoftware.pgpexample.util.PgpUtil.Companion.log
...
class PgpUtil {
    companion object {
         ...
         */
        internal const val BUFFER_SIZE = 1 shl 16
    }
}

@Suppress("unused")
private val init = initBcSecurityProvider()

/**
 * Init bc security provider
 */
private fun initBcSecurityProvider() {
...
fun verifySignature(
    ...
    var pgpFact = JcaPGPObjectFactory(PGPUtil.getDecoderStream(messageIn))
...
fun signEncryptInOnePass(
    ...
    val encryptedOut = encryptedDataGenerator.open(out, ByteArray(PgpUtil.BUFFER_SIZE))
    ...
    val literalOut = literalDataGenerator.open(
        ...
        PGPLiteralData.BINARY,
        embeddedFileName,
        Date(targetFile.lastModified()), ByteArray(PgpUtil.BUFFER_SIZE)
    )

    // read input file and write to target file using a buffer
    val buf = ByteArray(PgpUtil.BUFFER_SIZE)
    var len: Int
